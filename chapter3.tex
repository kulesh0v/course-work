...
\end{displaymath}
%ASK
Предположим, что системные матрицы неизвестны, но доступна одна траектория входа-выхода $\{u^d_k,y^d_k\}^{N-1}_{k=0}$ длины $N = 100$, которая генерируется путем равномерной выборки $u^d_k$ из $[-1,1]^2$.
